...
const createStorage = () => {
  ...
  const updateNodeInfo = ({ checked, producer, head_block_num }) => {
  ...
  const updateMissedProducing = (top21) => {
    const transformedProducers = top21.map(p => ({
      name: p.name,
      missedProducing: p.missedProducing,
    }));
    storage = storage
      .map(e => {
  ...
  const getAll = () => {
    process.send({ message: 'allData', data: storage });
  };
  const getUpdated = () => {
    storage = storage.map(e => ({ ...e, isUpdated: false }));
    const res = Object.values(updated);
    updated = {};
    process.send({ message: 'updated', data: res });
  };
  ...
  process.on('message', ({ message, data }) => {
    switch (message) {
      ...
      case 'updateMissedProducing': {
        updateMissedProducing(data);
        break;
      }
      case 'updateProducers': {
        updateProducers(data);
        break;
      }
