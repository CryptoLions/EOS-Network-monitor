 ...
 */
const exctractTransactions = async (block) => {
  ...
  return flatten(await Promise.all(txs
    .filter(({ trx: { transaction } }) => transaction)
    .map(({ trx: { id: txs_id, transaction: tx } }) => {
      const result = { txid: txs_id, producer: block.producer, blocknum };
      const { actions } = tx;

      return Promise.all(actions.map(async (action) => {
        const { data, name, account, authorization: [{ actor = '' } = {}] = [] } = action;

        const msgObject = { c1: blocknum, c2: name }; // , "txid": txs_id
        ...
        switch (name) {
          ...
          case 'setprods': {
            ...
            let newProds = '';
            mentionedAccounts.push(...data.schedule.map(schedule => {
          ...
          case 'issue': {
            ...
            msgObject.c6 = data.memo.toString().replace(lt, '&lt;').replace(gt, '&gt;').replace(ap, '&#39;')
              .replace(ic, '&#34;');
            txTo = data.to;
          ...
          case 'updateauth': {
            ...
            if (data.auth.keys.legth > 0) {
              if (data.auth.keys[0].key) { msgObject.c5 = data.auth.keys[0].key; }
            }

            if (data.auth.accounts.legth > 0) {
              if (data.auth.accounts[0].actor) { msgObject.c6 = data.auth.accounts[0].actor; }
            }
          ...
          case 'voteproducer': {
            ...
            let voteProds = '';
            mentionedAccounts.push(...data.producers.map(producer => {
              if (voteProds !== '') {
                voteProds += ', ';
              }
              voteProds += (producer);
          ...
          default: {
            msgObject.c3 = (account);
            msgObject.c4 = JSON.stringify(data).toString().replace(lt, '&lt;').replace(gt, '&gt;')
              .replace(ap, '&#39;')
              .replace(ic, '&#34;');
            msgObject.c5 = '';
            msgObject.c6 = '';
        ...
        if (account === 'eosio') {
          switch (name) {
            ...
            case 'bidname': {
              ...
              msgObject.c4 = data.newname.toString().replace(lt, '&lt;').replace(gt, '&gt;').replace(ap, '&#39;')
                .replace(ic, '&#34;');
        ...
        if (name === 'voteproducer') {
          ...
          txInfo.msgObject.c5 = `NET: ${stdout_obj.total_resources.net_weight} <BR>CPU: ${stdout_obj.total_resources.cpu_weight}<BR>`;
